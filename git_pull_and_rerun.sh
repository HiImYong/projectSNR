...
# 마이그레이트
docker exec snr bash -c "cd /data/site_projects/snr/src/ ; python manage.py migrate --settings=base.settings.prod"

# 장고를 운영모드로 실행
docker exec snr bash -c "cd /data/site_projects/snr/src ; nohup gunicorn --bind=0.0.0.0:8000 base.wsgi &"

# static collect 다시 수행
docker exec -it snr bash -c "cd /data/site_projects/snr/src ; echo yes | python manage.py collectstatic --settings=base.settings.prod"
